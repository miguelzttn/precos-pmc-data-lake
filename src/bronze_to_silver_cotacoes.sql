...
WITH
  ...
  (
    SELECT
      ...
      CAST(COALESCE(TRY_CAST(ds.codigo_categoria AS INT), 0) AS BIGINT) AS cd_categoria,
      CAST(ds.id_produto AS BIGINT) AS cd_produto,
      CAST(ds.preco_regular AS DECIMAL(11,2)) AS vl_preco_regular,
      CAST(ds.preco_atacado AS DECIMAL(10, 2)) AS vl_preco_atacado,
      CAST(ds.preco_atacado_qtd AS DECIMAL(10, 0)) AS vl_preco_atacado_quantidade,
      CAST(ds.preco_promocao AS DECIMAL(10, 2)) AS vl_preco_promocao,
      CAST(ds.preco_fidelidade AS DECIMAL(10, 2)) AS vl_preco_fidelidade
    FROM desired_subset ds
  )
